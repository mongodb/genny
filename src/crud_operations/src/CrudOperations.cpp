...
class WriteOperation : public BaseOperation {
...
class CreateIndexOperation : public BaseOperation {
...
class InsertOneOperation : public WriteOperation {
...
class UpdateOneOperation : public WriteOperation {
...
class UpdateManyOperation : public WriteOperation {
...
class DeleteOneOperation : public WriteOperation {
...
class DeleteManyOperation : public WriteOperation {
...
class ReplaceOneOperation : public WriteOperation {
...
// Maps the WriteCommand name to the conclassor of the designated Operation class.
std::unordered_map<std::string, WriteOpCallback&> bulkWriteConclassors = {
 ...
 *        OnSession: false
 */
class BulkWriteOperation : public BaseOperation {
    ...
    void createOps(const Node& writeOp, PhaseContext& context, ActorId id) {
        ...
        auto writeOpConclassor = bulkWriteConclassors.find(writeCommand);
        if (writeOpConclassor == bulkWriteConclassors.end()) {
            BOOST_THROW_EXCEPTION(InvalidConfigurationException(
                "WriteCommand '" + writeCommand + "' not supported in bulkWrite operations."));
        }
        auto createWriteOp = writeOpConclassor->second;
        _writeOps.push_back(
 ...
 *        OnSession: false
 */
class CountDocumentsOperation : public BaseOperation {
...
class EstimatedDocumentCountOperation : public BaseOperation {
...
class FindOperation : public BaseOperation {
...
class FindOneOperation : public BaseOperation {
...
class FindOneAndUpdateOperation : public BaseOperation {
...
class FindOneAndDeleteOperation : public BaseOperation {
...
class FindOneAndReplaceOperation : public BaseOperation {
 ...
 *        - { b : 1 }
 */
class InsertManyOperation : public BaseOperation {
...
class StartTransactionOperation : public BaseOperation {
...
class CommitTransactionOperation : public BaseOperation {
...
class WithTransactionOperation : public BaseOperation {
...
private:
    void createTxnOps(const Node& txnOp, PhaseContext& context, ActorId id) {
        ...
        auto opConclassors = getOpConclassors();
        auto opConclassor = opConclassors.find(opName);
        if (opConclassor == opConclassors.end()) {
            BOOST_THROW_EXCEPTION(InvalidConfigurationException(
                "Operation '" + opName + "' not supported inside a 'with_transaction' operation."));
        }
        auto createOp = opConclassor->second;
...
class SetReadConcernOperation : public BaseOperation {
 ...
 *            Level: majority
 */
class DropOperation : public BaseOperation {
...
std::unordered_map<std::string, OpCallback&> getOpConclassors() {
    // Maps the yaml 'OperationName' string to the appropriate conclassor of 'BaseOperation' type.
    std::unordered_map<std::string, OpCallback&> opConclassors = {
