...
QUERY_COUNT=100000
assert DOCUMENT_COUNT % (N_THREADS * BATCH_SIZE) == 0
assert QUERY_COUNT % (N_THREADS * BATCH_SIZE) == 0
...
def generate_rc_workloads(is_local):
    ...
    for ex in experiments:
        ...
        with open(f'workloads/{wldir}/{name}.yml', 'w') as f:
            f.write(template.render(document_count=DOCUMENT_COUNT, insert_threads=N_THREADS,
                query_count=QUERY_COUNT, query_threads=N_THREADS, batch_size=BATCH_SIZE,
...
def generate_rc_config_file():
    ...
    with open('generated/rc_perfconfig.yml', 'w') as f:
        f.write(template.render(patch_id=patch_id, experiments = experiments, n_threads=N_THREADS))
