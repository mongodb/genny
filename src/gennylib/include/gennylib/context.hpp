 ...
 */
template <class Out, bool Required = true> struct MaybeOptional {
  using type =
      typename std::conditional<Required, Out, std::optional<Out>>::type;
};
...
public:
  ...
  template <class T> void add(const T &elt) { _elements.push_back(elt); }
...
// helper below
template <class Out, class Current, bool Required = true,
...
OutV get_helper(const ConfigPath &parent, const Current &curr) {
  ...
  try {
    if
      constexpr(Required) { return curr.template as<Out>(); }
    else {
...
OutV get_helper(ConfigPath &parent, const Current &curr, PathFirst &&pathFirst,
                PathRest &&... rest) {
  if (curr.IsScalar()) {
    std::stringstream error;
    error << "Wanted [" << parent << pathFirst << "] but [" << parent
          << "] is scalar: [" << curr << "]";
    throw InvalidConfigurationException(error.str());
  }
  ...
  return V1::get_helper<Out, Current, Required>(
...
public:
   ...
   */
  WorkloadContext(YAML::Node node, metrics::Registry &registry,
                  Orchestrator &orchestrator,
                  const std::vector<ActorProducer> &producers)
      : _node{std::move(node)}, _registry{&registry},
    ...
    // TODO: Replace by a context get and use this value as the default value.
    ...
    _actors = constructActors(producers, _actorContexts, rng);
  }
   ...
   * @see get_static()
   */
  template <typename T = YAML::Node, bool Required = true,
            class OutV = typename V1::MaybeOptional<T, Required>::type,
            class... Args>
  OutV get(Args &&... args) const {
    return WorkloadContext::get_static<T, Required>(
   ...
   * drivers.
   */
  constexpr const ActorVector &actors() const { return _actors; }
...
private:
  ...
  static ActorVector
  constructActors(const std::vector<ActorProducer> &producers,
                  const std::vector<std::unique_ptr<ActorContext>> &,
                  std::mt19937_64 &);
  static std::vector<std::unique_ptr<ActorContext>>
  constructActorContexts(const YAML::Node &, WorkloadContext *);
  YAML::Node _node;
...
public:
  ...
  OutV get(Args &&... args) const {
    V1::ConfigPath p;
    return V1::get_helper<T, YAML::Node, Required>(p, _node,
                                                   std::forward<Args>(args)...);
  };
   ...
   */
  WorkloadContext &workload() { return *this->_workload; }
   ...
   */
  template <class... Args> constexpr auto timer(Args &&... args) const {
   ...
   */
  template <class... Args> constexpr auto gauge(Args &&... args) const {
   ...
   */
  template <class... Args> constexpr auto counter(Args &&... args) const {
    return this->_workload->_registry->counter(std::forward<Args>(args)...);
  }

  // Convenience forwarders for Orchestrator

  auto morePhases() { return this->_workload->_orchestrator->morePhases(); }
  ...
  auto abort() { return this->_workload->_orchestrator->abort(); }

  mongocxx::pool::entry client() { return _workload->_clientPool.acquire(); }
