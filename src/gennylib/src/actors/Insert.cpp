...
struct genny::actor::Insert::Config {
  ...
  struct PhaseConfig {
    PhaseConfig(const std::string collection_name,
                const YAML::Node document_node, const mongocxx::database &db)
        : collection{db[collection_name]}, json_document{
                                               makeDoc(document_node)} {}
...
void genny::actor::Insert::doPhase(int currentPhase) {
  ...
  BOOST_LOG_TRIVIAL(info) << _fullName << " Inserting "
...
genny::actor::Insert::Insert(genny::ActorContext &context,
    ...
    : PhasedActor(context, thread), _rng{context.workload().actors()},
      ...
      _config{std::make_unique<Config>(
          context, (*_client)[context.get<std::string>("Database")])} {}

genny::ActorVector
genny::actor::Insert::producer(genny::ActorContext &context) {
