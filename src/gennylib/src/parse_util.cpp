...
void checkTemplates(
  ...
  if (templates.count(key) > 0) {
    ...
    BOOST_LOG_TRIVIAL(trace)
        << "Pushing override for name: " << path << " and entry " << entry;
    overrides.push_back(std::make_tuple(path, key, entry));
  }
}

bsoncxx::document::value parseMap(
    YAML::Node node, std::set<std::string> templates, std::string prefix,
    std::vector<std::tuple<std::string, std::string, YAML::Node>> &overrides) {
  ...
  for (auto entry : node) {
    ...
    BOOST_LOG_TRIVIAL(trace)
        << "About to call checkTemplates and key is " << key;
    checkTemplates(key, entry.second, templates, prefix, overrides);
    if (entry.second.IsMap()) {
      docbuilder << key << open_document
                 << concatenate(parseMap(entry.second, templates,
                                         prefix + key + ".", overrides))
                 << close_document;
    } else if (entry.second.IsSequence()) {
      docbuilder << key << open_array
                 << concatenate(parseSequence(entry.second, templates,
                                              prefix + key + ".", overrides))
                 << close_array;
    } else { // scalar
...
bsoncxx::array::value parseSequence(
  ...
  for (auto entry : node) {
    if (entry.IsMap()) {
      arraybuilder << open_document << concatenate(parseMap(entry))
                   << close_document;
    } else if (entry.IsSequence()) {
      arraybuilder << open_array << concatenate(parseSequence(entry))
                   << close_array;
    } else // scalar
    {
      string doc = "{\"\": " + quoteIfNeeded(entry.Scalar()) + "}";
      BOOST_LOG_TRIVIAL(trace)
          << "In parseSequence and have scalar. Doc to pass to from_json: "
          << doc;
...
bsoncxx::array::value yamlToValue(YAML::Node node) {
  ...
  if (node.IsScalar()) {
    ...
    BOOST_LOG_TRIVIAL(trace)
        << "In yamlToValue and have scalar. Doc to pass to from_json: " << doc;
    BOOST_LOG_TRIVIAL(trace)
        << "In yamlToValue and have scalar. Original value is : "
        << node.Scalar();
