...
namespace mwg {
    ...
    insert_many::insert_many(YAML::Node &node) {
        ...
        if (node["type"].Scalar() != "insert_many") {
                cerr << "Insert_Many constructor but yaml entry doesn't have type == insert_many" << endl;
                exit(EXIT_FAILURE);
            }
        if (!node["container"].IsSequence()) {
            cerr << "Insert_Many constructor but yaml entry for container isn't a sequence" << endl;
            exit(EXIT_FAILURE);
        }
