...
TEST_CASE_METHOD(MongoTestFixture,
    ...
    SECTION("Moves a random chunk to a random shard.") {
        try {
            ...
            auto uuid = collectionDocOpt.value().view()["uuid"].get_binary();
            ...
            auto initialShardId = chunkOpt.value().view()["shard"].get_string().value.to_string();
            ...
            auto finalShardId =
                afterMigrationChunkOpt.value().view()["shard"].get_string().value.to_string();
