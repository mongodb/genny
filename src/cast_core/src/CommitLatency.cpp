...
struct CommitLatency::PhaseConfig {
    ...
    PhaseConfig(PhaseContext& phaseContext, const mongocxx::database& db, ActorId actorId)
        : collection{db[phaseContext["Collection"].to<std::string>()]},
          ...
          useSession{phaseContext["Session"].maybe<bool>().value_or(false)},
          useTransaction{phaseContext["Transaction"].maybe<bool>().value_or(false)},
...
CommitLatency::CommitLatency(genny::ActorContext& context)
    : Actor(context),
      ...
      _loop{context, CommitLatency::id(), (*_client)[context["Database"].to<std::string>()], CommitLatency::id()} {}
