...
#include <cast_core/actors/MinhActor.hpp>
...
// them to the `_loop{}` initializer in the `MinhActor` constructor
...
void MinhActor::run() {
    ...
    for (auto&& config : _loop) {
        ...
        for (const auto&& _ : config) {
            ...
            // debug level.
            //
            BOOST_LOG_TRIVIAL(debug) << " MinhActor Inserting "
...
MinhActor::MinhActor(genny::ActorContext& context)
    : Actor{context},
      _totalInserts{context.operation("Insert", MinhActor::id())},
      ...
      //
      _loop{context, (*_client)[context["Database"].to<std::string>()], MinhActor::id()} {}
...
// in the header file.
//
auto registerMinhActor = Cast::registerDefault<MinhActor>();
}  // namespace
}  // namespace genny::actor
