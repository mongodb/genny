 ...
 * ```
 *
 * Or you can fill out the generated workloads/docs/GetMoreActor.yml
 * file with extended documentation. If you do this, please mention
 * that extended documentation can be found in the docs/GetMoreActor.yml
 * file.
 *
...
class GetMoreActor : public Actor {
    ...
    // of the work is in the associated `GetMoreActor.cpp` file and its
    // assocated `GetMoreActor_test.cpp` integration-test file.
    //
...
public:
    ...
    //
    // This is how Genny knows that `Type: GetMoreActor` in workload YAMLs
...
private:
    ...
    // `GetMoreActor.cpp` implementation file.
    //
