...
void format_elmenent(boost::format& message, bsoncxx::array::element val) {
    switch (val.type()) {
...
public:
    FormatStringGenerator(const Node& node,
        ...
        : _rng{generatorArgs.rng},
          _fmtGen{stringGenerator(node["format"], generatorArgs)}
    {
        if (!node["array"].isSequence()) {
            std::stringstream msg;
        ...
        for (const auto&& [k, v] : node["array"]) {
            _parts.push_back(valueGenerator<false, UniqueAppendable>(v, generatorArgs, parsers));
        }
    }
    std::string evaluate() override {
        boost::format message(_fmtGen->evaluate());
        bsoncxx::builder::basic::array builder{};
        int64_t size = _parts.size();
        for (int64_t i = 0; i < size; i++) {
            auto&& valueGen = _parts[i];
            valueGen->append(builder);
        }

        for (auto val : builder.view()) {
            format_elmenent(message, val);
        }
        return message.str();
    }
...
protected:
    DefaultRandom& _rng;
    std::vector<UniqueAppendable> _parts;
